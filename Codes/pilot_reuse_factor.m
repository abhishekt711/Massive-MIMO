...
SNRvalues = [10^(20/10)]; %0 dB and 20 dB
...
for mind = 1:length(Mvalues)
    ...
    upper7 = Kvalues.*max([(1-7*Kvalues/tau_c); zeros(size(Kvalues))]).*mean(averageSEsReuse4_MR(:,:,mind),1);
    upperMR = max([upper1; upper3; upper4; upper7],[],1);
    
    p=plot(Kvalues,upper1,'o-');
    p.MarkerSize = 5;
    p.MarkerIndices = 1:10:length(upper1);
    %p.MarkerEdgeColor='black';
    %p.MarkerFaceColor=[0 0 0];
    q=plot(Kvalues,upper3,'d-');
    q.MarkerSize = 5;
    q.MarkerIndices = 1:10:length(upper1);
    %q.MarkerEdgeColor='black';
    %q.MarkerFaceColor=[0 0 0];
    r=plot(Kvalues,upper4,'s-');
    r.MarkerSize = 5;
    r.MarkerIndices = 1:10:length(upper1);
    s=plot(Kvalues,upper7,'^-');
    s.MarkerSize = 5;
    s.MarkerIndices = 1:10:length(upper1);
    
end

legend('F=1','F=3','F=4','F=7','Location','NorthWest');
xlabel('Number of Users (K)');
ylabel('Spectral Efficiency (bits/s/Hz/cell)');
...
for mind = 1:length(Mvalues)
    ...
    upper7 = Kvalues.*max([(1-7*Kvalues/tau_c); zeros(size(Kvalues))]).*mean(averageSEsReuse4_ZF(:,:,mind),1);
    upperZF = max([upper1; upper3; upper4],[],1);
    
%     a=plot(Kvalues,upper1,'r-');
%     b=plot(Kvalues,upper3,'k-.');
%     c=plot(Kvalues,upper4,'b--');
%     d=plot(Kvalues,upper7,'b--');
    a=plot(Kvalues,upper1,'o-');
    a.MarkerSize = 5;
    a.MarkerIndices = 1:10:length(upper1);
    %p.MarkerEdgeColor='black';
    %p.MarkerFaceColor=[0 0 0];
    b=plot(Kvalues,upper3,'d-');
    b.MarkerSize = 5;
    b.MarkerIndices = 1:10:length(upper1);
    %q.MarkerEdgeColor='black';
    %q.MarkerFaceColor=[0 0 0];
    c=plot(Kvalues,upper4,'s-');
    c.MarkerSize = 5;
    c.MarkerIndices = 1:10:length(upper1);
    d=plot(Kvalues,upper7,'^-');
    d.MarkerSize = 5;
    d.MarkerIndices = 1:10:length(upper1);
